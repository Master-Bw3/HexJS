...
public class ForgeMixinConfigPlugin implements IMixinConfigPlugin {
    ...
    public boolean shouldApplyMixin(String targetClassName, String mixinClassName) {
        if (mixinClassName.equals("io.github.master_bw3.hexjs.forge.mixin.MixinDatagenMain")) {
            var shouldApply = System.getProperty("hexjs.apply-datagen-mixin", "false").equals("true");
            if (shouldApply) {
                HexJs.LOGGER.error("Applying scuffed datagen mixin. This should not happen if not running datagen!");
            }
            return shouldApply;
        }
        return true;
